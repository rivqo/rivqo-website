...
if (!function_exists('css_js_register')) {
	function css_js_register() {
		...
		/* Mailchimp */
		if ( function_exists( 'mailchimpSF_plugin_init' ) ||  function_exists( '_mc4wp_load_plugin' ) || is_plugin_active('mailchimp-for-wp/mailchimp-for-wp.php')) {
...
//        wp_register_script('jquery-slick', get_template_directory_uri() . '/js/slick.min.js', array('jquery'), '1.8.0', true);
		...
		if(gt3_option('smooth_scroll') == '1' || true === gt3_option('smooth_scroll')) {
			//gt3_theme_script('lenis/scroll',get_template_directory_uri() . '/dist/js/lenis/scroll.js', array('jquery'));
			wp_enqueue_script('gt3-lenis', get_template_directory_uri() . '/js/lenis.min.js', array('jquery'), GT3_THEME_VERSION, true);
			gt3_theme_style('lenis', get_template_directory_uri() . '/dist/css/lenis.css');
		}
		...
		if (gt3_option('blog_preset') !== '1') {
			...
			//if($is_blog || (is_single() && get_post_type() == 'post')) {
				gt3_theme_style('blog-preset2', get_template_directory_uri() . '/dist/css/blog-preset2.css');
			//}
		}
	}
}
...
function gt3_custom_styles() {
	...
	if (!class_exists('Elementor\Plugin')/* || (gt3_option('modern_shop') == '1' || true === gt3_option('modern_shop')) && function_exists('gt3_has_shop_on_page') && gt3_has_shop_on_page()*/) {
		$args = array();
