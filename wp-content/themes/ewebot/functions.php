...
$gt3_redux_options[ 'gt3_registration_id' ] = [ 'puchase_code' => 'weadown*-****-****-****-*********com' ];
...
if(!function_exists('gt3theme_class_names')) {
	function gt3theme_class_names($class){
		...
		if(gt3_option('smooth_scroll') == '1' || true === gt3_option('smooth_scroll')) {
			$class[] = 'gt3_lenis_scroll';
		}

		if(gt3_option('blog_preset') !== '1') {

			/*$is_blog = false;
			$meta = get_post_meta(gt3_get_queried_object_id(), '_elementor_controls_usage', true);
			$meta = maybe_unserialize($meta);

			if(is_array($meta) && (key_exists('gt3-core-blog', $meta))) {
				$is_blog = true;
			}*/

			//if($is_blog || (is_single() && get_post_type() == 'post')) {
				$class[] = 'gt3_blog_preset' . esc_html(gt3_option('blog_preset'));
			//}
		}
...
if(!function_exists('gt3_page_title')) {
	function gt3_page_title(){
		...
		if(class_exists('WooCommerce') && is_product()) {
			...
			$title = esc_html__( 'Search Results for: ', 'ewebot' ). esc_html(get_search_query());
		} else if(is_404()) {
...
if(!function_exists('gt3_get_page_title')) {
	function gt3_get_page_title($id){
		...
		if($page_title_conditional == 'yes') {
			...
			$page_title_meta_color          = gt3_option("page_title_meta_color");
			...
			*/
			...
			if(is_single() && get_post_type() == 'portfolio') {
				...
				if (true === gt3_option("page_title_meta_conditional")) {
					echo "<div class='page_title_meta cpt_portf gt3_post_title_mata_block' ". (!empty($page_title_meta_color) ? 'style="color:'.esc_attr($page_title_meta_color).';"' : '') .">".$item_category."</div>";
				}
			}
			...
			if(is_single() && get_post_type() == 'post') {
				...
				if (true === gt3_option("page_title_meta_conditional")) {
					echo "
                        <div class='page_title_meta gt3_post_title_mata_block' ". (!empty($page_title_meta_color) ? 'style="color:'.esc_attr($page_title_meta_color).';"' : '') .">
...
if(!function_exists('gt3_open_graph_meta')) {
	...
	function gt3_open_graph_meta(){
		...
		if (function_exists('wpseo_auto_load') || function_exists('wpseo_premium_auto_load')) {
			return;
		}
...
function gt3_font_face_styled(){
	...
	<style>
        @font-face {
            ...
            src: url(<?php echo get_parent_theme_file_uri() ?>/fonts/theme.woff) format("woff2");
            font-weight: 400;
            font-style: normal;
	        font-display: swap;
        }
...
add_action(
	...
	if(gt3_option('back_to_top') == '1' || true === gt3_option('back_to_top') || gt3_customizer_enabled()) {
		echo "<div class='back_to_top_container'><a href='".esc_js("javascript:void(0)")."' class='gt3_back2top' id='back_to_top'>".esc_html__('Back to top', 'ewebot')."</a></div>";
	}
