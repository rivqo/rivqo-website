...
add_action('init', function(){
	...
	if(!!gt3_option('optimize_woo')) {
			...
			if(function_exists('is_woocommerce')) {
				...
				if(!is_woocommerce() && !is_cart() && !is_account_page() && !is_checkout() && !(function_exists('gt3_has_shop_on_page') && gt3_has_shop_on_page())) {
					...
					if(class_exists('YITH_WCWL'))	{
						...
						wp_deregister_style('yith-wcwl-add-to-wishlist' );
						wp_dequeue_script('yith-wcwl-add-to-wishlist-gutenberg');
						wp_dequeue_script('lapilli-ui-date');
						wp_dequeue_script('lapilli-ui-styles');
						wp_dequeue_script('lapilli-ui-components');
		...
		add_action('template_redirect', 'gt3_remove_woocommerce_inline_fonts');
	}
});
...
function gt3_remove_woocommerce_inline_fonts() {
	ob_start(function($content) {
		// Remove specific inline styles for fonts
		$pattern = "/<style class='wp-fonts-local' type='text\/css'>.*?<\/style>/s";
