		...
		if(isset($settings['header_logo']['url']) && !empty($settings['header_logo']['url'])) {
			...
			if($header_logo) {
				...
				if (!is_null($header_logo_array)) {
					$header_logo   = '<img class="wpda-builder-logo" src="'.esc_url($header_logo).'" alt="'.esc_attr($header_logo_array['alt']).'" title="'.esc_attr($header_logo_array['title']).'" width="'.esc_attr($header_logo_array['width']).'" height="'.esc_attr($header_logo_array['height']).'"/>';
					$enable_sticky = true;
				}  else {
					$header_logo = '';
				}
		...
		if(isset($settings['logo_sticky']['url']) && !empty($settings['logo_sticky']['url'])) {
			...
			if($logo_sticky) {
				...
				if (!is_null($sticky_logo_array)) {
					$logo_sticky = '<img class="wpda-builder-logo_sticky" src="'.esc_url($logo_sticky).'"  alt="'.esc_attr($sticky_logo_array['alt']).'" title="'.esc_attr($sticky_logo_array['title']).'" width="'.esc_attr($sticky_logo_array['width']).'" height="'.esc_attr($sticky_logo_array['height']).'" />';
				} else {
					$logo_sticky = '';
				}
			}
