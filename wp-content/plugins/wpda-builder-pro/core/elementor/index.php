...
class Elementor {
		...
		if(is_array($this->widgets) && !empty($this->widgets)) {
			foreach($this->widgets as $module) {
				...
				if(class_exists($module)) {
					$widgets_manager->register(new $module());
				}
			}
		}
	}
	...
	public function editor_enqueue_styles(){
		...
		if($document instanceof Library_Basic) {
			if (apply_filters('wpda-builder/editor/elementor/library/assets', true)) {
			...
			if (apply_filters('wpda-builder/editor/elementor/posts/assets', true)) {
