...
if(!class_exists('ElementorModal\Widgets\GT3_Core_Elementor_Widget_Portfolio')) {
	class GT3_Core_Elementor_Widget_Portfolio extends \ElementorModal\Widgets\GT3_Core_Widget_Base {
		...
		public function ajax_handler(){
			...
			if(isset($_POST['show_date']) && ($_POST['show_date'] === true || $_POST['show_date'] == 'true')) {
				$_POST['show_date'] = true;
			} else {
			...
			while($query->have_posts()) {
				$query->the_post();
				$respond .= $this->renderItem($_POST['use_filter'], $_POST['show_title'], $_POST['show_category'], $_POST['show_date'], $render_index, $_POST['settings']);
		...
		public function renderItem($use_filter = false, $show_title = false, $show_category = false, $show_date = false, $render_index = 0, $settings = false){
			$item_class                = '';
			$item_category             = '';
			...
			if(!$image_id) {
				...
				$image_src = (is_array($image_src) && !empty($image_src[0])) ? $image_src[0] : '';
			...
			$render .= '<a href="'.esc_url($linkToTheWork).'" class="lightbox" '.$target.'>';
			$render .= '<div class="img_wrap"><div class="img" >';
			$render .= $image;
			$render .= '</div></div>';

			if(((bool) $show_title || (bool) $show_category  || (bool) $show_date || (bool) $show_description) && (!empty($title) || !empty($item_category) || !empty($post_descr))) {
				$render .= '<div class="text_wrap">';
				if((bool) $show_title && !empty($title)) {
					$render .= '<'. apply_filters('gt3_portfolio_packery_title_tag', Utils::validate_html_tag( $settings['html_tag'] )) .' class="title">'.get_the_title().'</'. apply_filters('gt3_portfolio_packery_title_tag', Utils::validate_html_tag( $settings['html_tag'] )) .'>';
				}
				if(((bool) $show_category && !empty($item_category)) || (bool) $show_date) {
					$categories_block_class = '';
					if(((bool) $show_category && !empty($item_category)) && (bool) $show_date) {
						$categories_block_class = ' gt3_port_meta';
					}
					$render .= '<div class="categories' . esc_html($categories_block_class) . '">';
					if((bool) $show_category && !empty($item_category)) {
						$render .= '<div class="categories_inner">'.$item_category.'</div>';
					}
					if((bool) $show_date) {
						$render .= '<div class="gt3_port_date">'.esc_html(get_the_time(get_option( 'date_format' ))).'</div>';
					}
					$render .= '</div>';
				}
			...
			if($settings) {
				...
				if(!empty($image_src[0]) && strlen($image_src[0])) {
					...
					if($show_type == 'packery') {
						...
						$ration = apply_filters('gt3/custom_packery_ration', $ration, $settings, $packery_grids, $render_index);
