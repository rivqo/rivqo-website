...
$graph_value = ((isset($settings['graph_value']['size'])) ? ((int)$settings['graph_value']['size'])/100 : '');

if (!empty($settings['label_value'])) {
	$label_value = $settings['label_value'];
} else {
	$label_value = ((isset($settings['graph_value']['size'])) ? $settings['graph_value']['size'] : '');
}
...
?>
	<div <?php echo ''.$widget->get_render_attribute_string('wrapper') ?>>
		<div
			  ...
			  data-size="<?php echo ((isset($settings['graph_size']['size'])) ? (int)($settings['graph_size']['size']) : ''); ?>"
			  data-thickness="<?php echo ((isset($settings['graph_thickness']['size'])) ? (int)($settings['graph_thickness']['size']) : ''); ?>"
			...
			>
			<strong class="element_typography">fffffff<?php echo (int)$label_value . '<span>' . esc_attr($settings['chart_units']) . '</span>' ?></strong>
			<?php
