		...
		public function get_img_url ($image_src = false,$settings = false,$title = false,$render_index = false){
			if ($settings) {
	            ...
	            if (!empty($wp_get_attachment_url)) {
	            	...
	            	$featured_image .= '<img ' . $gt3_featured_image_url . ' alt="'.esc_attr($alt_text).'" '.(!empty($gt3_featured_image_class) ? ' class="'.esc_attr($gt3_featured_image_class).'"' : '').'/>';
	            }else{
