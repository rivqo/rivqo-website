<?php
	...
	class GT3ServiceRegister {
		...
		private function __construct() {
			...
			$slug_option        = function_exists( 'gt3_option' ) ? gt3_option( 'service_slug' ) : '';
			$this->slug         = empty( $slug_option ) ? $this->cpt : sanitize_title( $slug_option );

			$this->single_label  = apply_filters( "gt3_service_single_label_filter", esc_html__('Service', 'gt3_themes_core'));
	        $this->multiple_label  = apply_filters( "gt3_service_multiple_label_filter", __('Services', 'gt3_themes_core'));
		...
		public function register_archive_template($archive){
			...
			if(is_archive() && $term instanceof \WP_Term && property_exists($term, 'taxonomy' ) && in_array($term->taxonomy, array($this->taxonomy, $this->tag))) {
				if(!file_exists(get_template_directory().'/archive-'.$this->cpt.'.php')
	            	...
	            	return plugin_dir_path(dirname( __FILE__ )).'/service/archive-'.$this->cpt.'.php';
	            }
	        }
		...
		public function shortcode_gallery($content, $attr, $instance){
			...
			if(get_post_type() != $this->cpt) {
				...
				<?php
				$GLOBALS['gt3themes_service_gallery_footer'] = ob_get_clean();
			}
		...
		public function wp_footer(){
			if (isset($GLOBALS['gt3themes_service_gallery_footer']) && !empty($GLOBALS['gt3themes_service_gallery_footer'])) {
				echo $GLOBALS['gt3themes_service_gallery_footer'];
			}
		}

	}

	GT3ServiceRegister::instance();
