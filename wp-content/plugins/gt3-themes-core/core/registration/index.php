...
final class Registration {
	...
	private function __construct(){
		...
		$this->user_agent    = 'WordPress/'.$wp_version.'; '.apply_filters( 'wpml_permalink', site_url(), apply_filters( 'wpml_default_language', '' ) );
	...
	private function load(){
		...
		if($update) {
			...
			$log->save_log(array(
				'type'     => E_USER_WARNING,
				'message'  => 'Registration not found. '.($isMerged ? ' Merged.' : ''),
				'file'     => __FILE__,
				'line'     => __LINE__,
	...
	private function save($data = array(), $from_request = false){
		if ($data === true) {
			// force
			return update_option($this->option_key, json_encode($this->settings));
		}
		if($from_request) {
	...
	private function fetch($url, $data){
		$response = wp_remote_post($url, array(
			...
			'body'        => array_merge(array(
				...
				'ver'             => 2,
				'purchase_url'    => apply_filters( 'wpml_permalink', site_url(), apply_filters( 'wpml_default_language', '' ) ),
			), $data),
		));
