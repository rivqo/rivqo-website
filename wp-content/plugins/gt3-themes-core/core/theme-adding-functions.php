	...
	function gt3_carousel_column_elementor_controls($element, $section_id, $args){
		...
		if('container' === $element->get_name() && 'section_layout' === $section_id) {
			$element->start_controls_section(
				...
				[
					'tab'       => Elementor\Controls_Manager::TAB_LAYOUT,
					'label'     => __('GT3 Options', 'gt3_themes_core'),
				]
			);
			...
			$element->add_control(
				'gt3_pin_container',
				array(
					'label' => esc_html__('Enable Pin Container', 'gt3_themes_core'),
					'type'  => Elementor\Controls_Manager::SWITCHER,
			...
			$element->add_control(
				'gt3_pin_container_trigger',
				array(
					'label'        => esc_html__('Pin Wrapper', 'gt3_themes_core'),
					'type'         => Elementor\Controls_Manager::SELECT,
					'options'      => array(
						'' => esc_html__('Default', 'gt3_themes_core'),
			...
			$element->add_control(
				'gt3_pin_container_custom_area',
				array(
					'label'     => esc_html__('Custom Pin Area', 'gt3_themes_core'),
					'description'     => esc_html__('Add the section class where the element will be pin. Please use the parent section or container class.', 'gt3_themes_core'),
					'type'      => Elementor\Controls_Manager::TEXT,
					'placeholder'   => esc_html__('.pin_area', 'gt3_themes_core'),
					'ai'                 => false,
					'frontend_available' => true,
					'render_type'        => 'none',
					'condition' => array(
						'gt3_pin_container!' => '',
						'gt3_pin_container_trigger' => 'custom',
					),
				)
			);

			$element->add_control(
				'gt3_pin_container_end_trigger',
				array(
					'label'     => esc_html__('End Trigger', 'gt3_themes_core'),
					'type'      => Elementor\Controls_Manager::TEXT,
					'placeholder'   => esc_html__('.end_trigger', 'gt3_themes_core'),
					'ai'                 => false,
					'frontend_available' => true,
					'render_type'        => 'none',
					'condition' => array(
			...
			$element->add_control(
				'gt3_pin_container_area_start',
				array(
					'label'        => esc_html__('Start', 'gt3_themes_core'),
					'description'        => esc_html__('First value is element position, Second value is display position', 'gt3_themes_core'),
					'type'         => Elementor\Controls_Manager::SELECT,
					'options'      => array(
						'top top'       => esc_html__( 'Top Top', 'gt3_themes_core' ),
						'top center'    => esc_html__( 'Top Center', 'gt3_themes_core' ),
						'top bottom'    => esc_html__( 'Top Bottom', 'gt3_themes_core' ),
						'center top'    => esc_html__( 'Center Top', 'gt3_themes_core' ),
						'center center' => esc_html__( 'Center Center', 'gt3_themes_core' ),
						'center bottom' => esc_html__( 'Center Bottom', 'gt3_themes_core' ),
						'bottom top'    => esc_html__( 'Bottom Top', 'gt3_themes_core' ),
						'bottom center' => esc_html__( 'Bottom Center', 'gt3_themes_core' ),
						'bottom bottom' => esc_html__( 'Bottom Bottom', 'gt3_themes_core' ),
						'custom'        => esc_html__( 'custom', 'gt3_themes_core' ),
					),
					'separator'    => 'before',
					'default'      => 'top top',
			...
			$element->add_control(
				...
				array(
					'label'     => esc_html__('Custom', 'gt3_themes_core'),
					'type'      => Elementor\Controls_Manager::TEXT,
					'default'   => esc_html__('top top', 'gt3_themes_core'),
					'placeholder'   => esc_html__('top top+=100', 'gt3_themes_core'),
					'ai'                 => false,
					'frontend_available' => true,
					'render_type'        => 'none',
					'condition' => array(
						'gt3_pin_container!' => '',
						'gt3_pin_container_area_start' => 'custom',
					),
				)
			);

			$element->add_control(
				'gt3_pin_container_area_end',
				array(
					'label'        => esc_html__('End', 'gt3_themes_core'),
					'description'        => esc_html__('First value is element position, Second value is display position', 'gt3_themes_core'),
					'type'         => Elementor\Controls_Manager::SELECT,
					'options'      => array(
						'top top'       => esc_html__( 'Top Top', 'gt3_themes_core' ),
			...
			$element->add_control(
				'gt3_pin_container_area_end_custom',
				array(
					'label'     => esc_html__('Custom', 'gt3_themes_core'),
					'type'      => Elementor\Controls_Manager::TEXT,
					'default'   => esc_html__('bottom top', 'gt3_themes_core'),
					'placeholder'   => esc_html__('bottom top+=100', 'gt3_themes_core'),
					'ai'                 => false,
					'frontend_available' => true,
					'render_type'        => 'none',
					'condition' => array(
						'gt3_pin_container!' => '',
						'gt3_pin_container_area_end' => 'custom',
					),
				)
			);
			...
			foreach ( Elementor\Plugin::$instance->breakpoints->get_active_breakpoints() as $breakpoint_key => $breakpoint_instance ) {
				...
				/* translators: 1: Breakpoint label, 2: `>` character, 3: Breakpoint value. */
					esc_html__( '%1$s (%2$s %3$dpx)', 'gt3_themes_core' ),
			...
			$element->add_control(
				'gt3_pin_container_breakpoint',
				array(
					'label'              => esc_html__( 'Breakpoint', 'gt3_themes_core' ),
					'type'               => Elementor\Controls_Manager::SELECT,
					'separator'          => 'before',
					'description'        => esc_html__( 'Note: Choose at which breakpoint Pin element will work.', 'gt3_themes_core' ),
...
add_action('elementor/frontend/container/before_render', function( $element ) {
	if ( 'container' === $element->get_name() ) {
		...
		if(!empty($settings['gt3_pin_container'])) {
			Script::enqueue_core_asset('gsap');
			Script::enqueue_core_asset('scrolltrigger');
		}
	}
}, 10, 1);
		...
		if($show_share == "1" || true === $show_share) { ?>
			...
			<div class="post_share_block">
				...
				<div class="post_share_wrap">
					<ul>
						<?php
						echo '<li class="post_share-facebook"><a target="_blank" href="'.esc_url('https://www.facebook.com/share.php?u='.get_permalink()).'" data-title="' . esc_html__('Facebook', 'gt3_themes_core') . '"><span class="fa fa-facebook"></span></a></li>';
						echo '<li class="post_share-twitter"><a target="_blank" href="'.esc_url('https://twitter.com/intent/tweet?text='.get_the_title().'&amp;url='.get_permalink()).'" data-title="' . esc_html__('Twitter', 'gt3_themes_core') . '"><span class="fa fa-twitter"></span></a></li>';
						if(strlen($featured_image[0]) > 0) {
							echo '<li class="post_share-pinterest"><a target="_blank" href="'.esc_url('https://pinterest.com/pin/create/button/?url='.get_permalink().'&media='.$featured_image[0]).'" data-elementor-open-lightbox="no" data-title="' . esc_html__('Pinterest', 'gt3_themes_core') . '"><span class="fa fa-pinterest"></span></a></li>';
						}
						echo '<li class="post_share-linkedin"><a target="_blank" href="'.esc_url('https://www.linkedin.com/shareArticle?mini=true&url='.get_permalink().'&title='.esc_attr(get_the_title()).'&source='.get_bloginfo("name")).'" data-title="' . esc_html__('Linkedin', 'gt3_themes_core') . '"><span class="fa fa-linkedin"></span></a></li>';
						/* Email Link */
						ob_start();
						the_title('', '', true);
						...
						echo '<li class="post_share-mail"><a target="_blank" href="'.$email_link.'" data-title="' . esc_html__('Email', 'gt3_themes_core') . '"><span class="fa fa-envelope"></span></a></li>';
						?>
					</ul>
				</div>
			</div>
...
/* RS Slider */
if ( ! function_exists( 'gt3_download_slider_demo' ) ) {
	function gt3_download_slider_demo() {

		$encoded_url = apply_filters('gt3/rs/slider/url', '');

		if (empty($encoded_url)) {
			return;
		}

		$remote_file_url = base64_decode($encoded_url);

		$upload_dir = wp_upload_dir();
		$destination_path = get_template_directory() . '/core/demo-data/demo/' . apply_filters('gt3/rs/slider/name', '');

		if (file_exists($destination_path) || empty($destination_path)) {
			return;
		}

		$response = wp_remote_get($remote_file_url, array('timeout' => 300));

		if (is_wp_error($response)) {
			error_log('Error downloading demo content: ' . $response->get_error_message());
			return;
		}

		$response_code = wp_remote_retrieve_response_code($response);
		if ($response_code != 200) {
			error_log('Error downloading demo content: invalid response code ' . $response_code);
			return;
		}

		$file_contents = wp_remote_retrieve_body($response);
		if (!empty($file_contents)) {
			file_put_contents($destination_path, $file_contents);
		}
	}
}
add_action('gt3_homepage_importer_slider_name', 'gt3_download_slider_demo');
