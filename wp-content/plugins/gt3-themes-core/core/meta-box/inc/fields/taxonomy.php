...
class RWMB_Taxonomy_Field extends RWMB_Object_Choice_Field {
	...
	public static function ajax_get_terms() {
		...
		// Search.
		$field['query_args']['name__like'] = $request->filter_post( 'term', FILTER_SANITIZE_FULL_SPECIAL_CHARS );

		// Pagination.
		$limit = isset( $field['query_args']['number'] ) ? (int) $field['query_args']['number'] : 0;
		if ( 'query:append' === $request->filter_post( '_type', FILTER_SANITIZE_FULL_SPECIAL_CHARS ) ) {
