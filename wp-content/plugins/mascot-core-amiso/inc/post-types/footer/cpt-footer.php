<?php
namespace MASCOTCOREAMISO\CPT\Footer;
 ...
 * Singleton class
 * class CPT_Footer
 * @package MASCOTCOREAMISO\CPT\Footer;
 */
final class CPT_Footer implements Lib\Mascot_Core_Amiso_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Footer', 'mascot-core-amiso' );
		$this->ptPluralName = esc_html__( 'Parts - Footer', 'mascot-core-amiso' );
		$this->ptKey = 'footer';
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-footer'	=> esc_html__( 'Currently Active Footer', 'mascot-core-amiso' ),
		...
		switch( $columns ) {
			case 'active-footer' :
				if( mascot_core_amiso_theme_installed() ) {
					$active_footer_id = amiso_get_redux_option( 'footer-settings-choose-footer-widget-area', 'default' );
