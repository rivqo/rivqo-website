<?php
namespace MASCOTCOREAMISO\CPT\SidePushPanel;
 ...
 * @package MASCOTCOREAMISO\CPT\SidePushPanel;
 */
final class CPT_SidePushPanel implements Lib\Mascot_Core_Amiso_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Side Push Panel', 'mascot-core-amiso' );
		$this->ptPluralName = esc_html__( 'Parts - Side Push Panel', 'mascot-core-amiso' );
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-side-push-panel'	=> esc_html__( 'Currently Active Side Push Panel', 'mascot-core-amiso' ),
		...
		switch( $columns ) {
			case 'active-side-push-panel' :
				if( mascot_core_amiso_theme_installed() ) {
					$active_headertop_id = amiso_get_redux_option( 'header-settings-choose-side-push-panel-cpt-widget-area', 'default' );
