...
if(!class_exists('gt3_photo_video_galery_pro')) {
	class gt3_photo_video_galery_pro {
		...
		private function __construct(){
			...
			$plugin_info          = get_plugin_data(__DIR__.'/gt3-photo-video-gallery-pro.php', false, false);
		...
		private function version_compare(){
			...
			$lite_version = get_plugin_data(GT3PG_PLUGINPATH.'/gt3-photo-video-gallery.php', false, false);
		...
		public function mainActions(){
			...
			add_action(
				...
				function($current_screen){
					if($current_screen->base === 'gt3-gallery-pro_page_gt3pg_pro_license' ||
					   $current_screen->base === 'gt3-gallery-lite_page_gt3pg_pro_license') {
...
//				), 10, 2 );
		...
		private function inlineActions(){
			...
			add_filter('gt3pg_admin_version', function(){
				$plugin_info = get_plugin_data(GT3PG_PRO_FILE, false, false);
...
//								'blueimp-gallery-carousel' => 'blueimp-gallery-carousel',
		...
		public function menu_license_page(){
			...
			?>
			<div class="gt3_license_wrap">
				<div class="gt3_license_inner">
					<div class="gt3_license_settings">
						<h2><?php esc_html_e('License Management', 'gt3pg_pro'); ?></h2>
						<?php
						if(is_array($licenses) && count($licenses)) {
							echo '<div class="gt3_license_settings_inner">';
		...
		//								);
								...
								<div class="gt3pg_block_wrap_license">
									<form method="post" action="options.php">
										<div class="gt3pg_title">
											<?php echo esc_html($license['plugin_name']); ?>
										</div>
										<div class="gt3pg_key">
											<input type="text" value="<?php echo esc_attr($license['license']) ?>" name="<?php echo esc_attr($license['license_field']) ?>" />
		...
		public function gt3_render_hidden_admin_mix_tab_control_linkTo(){
			...
			$hiddent_lightbox = array(
				...
				'20' => '<div class="lightboxImageSize_full_hidden gt3pg_notice_red" style="display: none">'.
				        esc_html__('Not recommended. If the images are not optimized for the web, it can lead to slow page loading.', 'gt3pg_pro').
				        '</div>',
		...
		public function gt3_render_hidden_admin_mix_tab_control_imageSize(){
			...
			<div class="hidden_imageSize gt3pg_display_none" style="display: none;">
				<div class="gt3pg_optimized_full_hidden gt3pg_notice_red">
					<?php esc_html_e('Not recommended. If the images are not optimized for the web, it can lead to slow page loading.', 'gt3pg_pro') ?>
				</div>
			</div>
			<?php
	...
	<div class="gt3pg_play-pause"></div>
		...
		public function gt3_before_admin_panel_tabs_controls($panels){
			...
			$panels["10.2"] = new gt3panel_control(array(
				'title'  => __('Autoplay Duration (seconds)', 'gt3pg_pro'),
			...
			$panels["10.5"] = new gt3panel_control(array(
				'title'  => __('Image Scalling', 'gt3pg_pro'),
